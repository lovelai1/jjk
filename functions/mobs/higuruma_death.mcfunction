...
tp @s ~~~
scoreboard players add @a[scores={combat=1..},r=50,m=!spectator] expadd 225
scoreboard players add @a[scores={combat=1..},r=50,m=!spectator] guilty_score 3
execute unless score mobeventstoggle Mode = three Mode as @p[r=50] run structure load higurumas ~~~
